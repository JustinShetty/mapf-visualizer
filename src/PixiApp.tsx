...
const PixiApp = forwardRef(
    (
        ...
        const deliveredMarkersRef = useRef<PIXI.Container>(
            new PIXI.Container()
        );
        ...
        const moveAndRotateSprites = useCallback(
            (agents: PIXI.Container[], currentTime: number) => {
                ...
                const nextState =
                    solution[
                        Math.min(currentTimestep + 1, solution.length - 1)
                    ];
                ...
                agents.forEach((agent, index) => {
                    ...
                    const startRotation = orientationToRotation(
                        startPose.orientation
                    );
                    const endRotation = orientationToRotation(
                        endPose.orientation
                    );
        ...
        const updatePaths = useCallback(
            (agents: PIXI.Container[], currentTime: number) => {
                ...
                agents.forEach((_agent, index) => {
                    ...
                    const partial_segments = agentPathsRef.current.partial
                        .children[index] as PIXI.Container;

                    while (full_segments.children.length > currentTimestep) {
                        if (full_segments.children.length === 0) break;
                        full_segments.removeChildAt(
                            full_segments.children.length - 1
                        );
                    }
                    ...
                    while (full_segments.children.length < currentTimestep) {
                        ...
                        const segment = full_segments.addChild(
                            new PIXI.Graphics()
                        );
                        segment.moveTo(
                        ...
                        segment.lineTo(
                            scalePosition(
                                solution[segIndex + 1][index].position.x
                            ),
                            scalePosition(
                                solution[segIndex + 1][index].position.y
                            )
                        );
                    ...
                    if (
                        ...
                        const segment = partial_segments.addChild(
                            new PIXI.Graphics()
                        );
                        ...
                        const interpolatedPosition = {
                            x:
                                solution[currentTimestep][index].position.x +
                                (solution[currentTimestep + 1][index].position
                                    .x -
                                    solution[currentTimestep][index].position
                                        .x) *
                                    interpolationProgress,
                            y:
                                solution[currentTimestep][index].position.y +
        ...
        const animateSolution = useCallback(() => {
            ...
            if (tickerCallbackRef.current) {
                ...
                if (goalMarkersRef.current)
                    goalMarkersRef.current.removeChildren();
                if (goalVectorsRef.current)
                    goalVectorsRef.current.removeChildren();
            }
            ...
            solution[0].forEach((_pose, agentId) => {
                ...
                if (orientationAware) {
                    ...
                    const triangle = circleContainer.addChild(
                        new PIXI.Graphics()
                    );
                    triangle
            ...
            const animate = () => {
                ...
                if (playAnimationRef.current === true) {
                    if (timestepRef.current < solution.length - 1) {
                        ...
                        timestepRef.current +=
                            stepSizeRef.current / approximateFramerate;
            ...
            if (app === null) {
                ...
                if (viewport === null) {
                    ...
                    if (app.stage.children.length === 0) {
                        ...
                        hudRef.current = app.stage.addChild(
                            new PIXI.Container()
                        );
        ...
        const drawGrid = useCallback(() => {
            ...
            for (let x: number = 0; x < graph.width; x++) {
                for (let y: number = 0; y < graph.height; y++) {
                    ...
                    const cellGraphic = cellContainer.addChild(
                        new PIXI.Graphics()
                    );
        ...
        useEffect(() => {
            ...
            if (goalMarkersRef.current)
                goalMarkersRef.current.visible = showGoals;
            if (goalVectorsRef.current)
